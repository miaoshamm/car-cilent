...
export const getHandOverCarInfo = (orderNo) => {
	return request({
		url: `/userOrder/findHandOverCarInfo?orderNo=${orderNo}`,
		method: "GET",
	});
};
...
// 录入泊车信息  
export const inputParkingRecord = (data) => {
	return request({
		url: `/servicer/toParkCarInfo`,
		method: "POST",
		data,
	});
};
...
// 客户到达订单地址 
export const UserGetToLocation = (userNo, orderNo) => {
	return request({
		url: `/userOrder/arrivalOrderAddress?orderNo=${orderNo}&userNo=${userNo}`,
		method: "GET"
	});
};