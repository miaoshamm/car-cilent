<template>
	<view style="height: 100vh; background: #f6f7f8; display: flex; flex-direction: column" class="">
		...
		<view v-if="!isInvoice" :style="{ flex: 1, padding: '32rpx', position: 'relative', overflow: 'auto' }">
			...
			<view style="margin-top: 7rpx" class="card">
				<view class="card_line" v-if="isServicer">
					...
					<view style="display: flex; align-items: center" class="card_line_content" @click="callUp(orderInfo.orderExtraVo?.reservationPhone)">
				...
				<view class="card_line" v-if="orderInfo?.geographicLocationVo?.addressType === 'OPTIONAL'">
					...
					<view class="card_line_content" style="display: flex; align-items: center">
						...
						<image src="../../static/images/order/location.png" mode="widthFix" style="margin-left: 16rpx; width: 36rpx"></image>
					</view>
				</view>
			...
			<view style="margin-top: 7rpx" class="card">
				...
				<view class="card_line" style="height: 144rpx" v-if="!isServicer">
					...
					<view class="card_line_content" style="display: flex; align-items: center" @click="callUp(orderInfo?.servicerVo?.servicerPhone)">
		...
		<view style="height: 128rpx; background-color: white; box-sizing: border-box; padding: 20rpx 32rpx" v-if="isServicer">
			<view
				style="
...
import { getOrderPaymentRecord, ServicerDestination } from '../../api/index.js';
...
const { order_no, isServicer } = defineProps(['order_no', 'isServicer']);
