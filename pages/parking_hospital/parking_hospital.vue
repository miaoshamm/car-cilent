<template>
  <view
    style="min-height: 100vh; background: #f6f7f8; display: flex; flex-direction: column"
    class="wrapper-p"
  >
    <u-navbar
      :autoBack="true"
      title="预约代客泊车"
      titleStyle="font-size:36rpx"
      placeholder
      safeAreaInsetTop
    ></u-navbar>
    <view style="flex: 1; padding: 32rpx; position: relative; overflow: scroll">
      <LicensePlateSelection
        :licensePlate="subscribeInfo.licensePlate"
        type="input"
      ...
      >
        <view class="card" style="margin-top: 19rpx">
          ...
          <u-form-item label="预约姓名" prop="userName" borderBottom>
            <u--input
              v-model="subscribeInfo.userName"
              border="none"
              placeholder="请输入预约姓名"
            ></u--input>
          </u-form-item>
            ...
            <template #right>
              <view style="display: flex; align-items: center" @click="openMap">
                ...
                <image
                  ...
                  mode="widthFix"
                  style="width: 32rpx; margin-left: 8rpx"
                ></image>
              </view>
            </template>
          </u-form-item>
          <u-form-item label="帮忙取车" prop="isHelpGet" borderBottom>
            <view style="width: 100%; display: flex; justify-content: flex-end">
              <u-switch
                v-model="subscribeInfo.isHelpGet"
                @change=""
                activeColor="#449656"
              ></u-switch>
            </view>
        ...
        <view class="card" style="margin-top: 19rpx">
          <u-form-item label="车辆颜色" prop="carColor" borderBottom>
            <u--input
              v-model="subscribeInfo.carColor"
              border="none"
              placeholder="请输入车辆颜色"
            ></u--input>
          </u-form-item>
          <u-form-item
            ...
            prop="carTypeName"
            borderBottom
            @click="subscribeInfo.isShowCarType = true"
          >
            <u--input
              disabled
              v-model="subscribeInfo.carTypeName"
              disabledColor="#ffffff"
              placeholder="请选择车辆型号"
              border="none"
            ></u--input>
  ...
  formRef.value.validate().then(async (res) => {
    ...
    if (res) {
      const orderInfo = await reservationParkOrder({
        ...
        orderType: subscribeInfo.value.isHelpGet
          ? "HELP_SAVE_AND_RETRIEVE"
          : "HELP_SAVE_ASK_OF",
        reservationTime: dayjs(subscribeInfo.value.reservationTime).format(
          "YYYY-MM-DD HH:mm:ss"
        ),
